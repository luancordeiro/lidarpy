...
def profile_read(f_name, dbin=0, dtime: float = 0, ach=0, maxz=0):
    head = {}
...
if __name__ == '__main__':
    ...
    files.sort()
    file = files[0]
    head, phy, raw = profile_read(f"{directory}/{file}", dtime=0.004)
    print(head)

    alt = np.arange(1, len(phy[0]) + 1) * 7.5

    for title, p in zip(head["ch"]["wlen"], phy):
        plt.plot(alt, p)
        plt.title(title)
        plt.grid()
        plt.show()
